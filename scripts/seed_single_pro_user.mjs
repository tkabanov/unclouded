/**
 * One-off Active Pro user for SUB-PRO-* E2E (avoids mutating sub-pro@test.com).
 *
 *   SUPABASE_SERVICE_ROLE_KEY=... node scripts/seed_single_pro_user.mjs
 *
 * Optional: SEED_EMAIL=sub-pro-run@test.com
 */
...
const EMAIL = process.env.SEED_EMAIL ?? "sub-pro-run@test.com";
...
const ONBOARDING_PROFILE = {
  ...
  results: {
    ...
    classification: {
      ...
      description: "Seeded for Pro subscription QA.",
...
async function main() {
  ...
  console.log(`Seeding ${EMAIL} (Active Pro)...`);
  ...
  const { error: syncError } = await admin.rpc("billing_sync_stripe_subscription", {
    ...
    p_stripe_customer_id: `cus_seed_pro_${userId.slice(0, 8)}`,
    p_stripe_subscription_id: `sub_seed_pro_${userId.slice(0, 8)}`,
    p_stripe_price_id: "price_seed_pro",
  });
