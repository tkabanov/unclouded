...
- Prefer **not** staging junk: `tmp_*`, `docs/tmp-*`, `scripts/__pycache__/`, `frontend/node_modules/`, `.claude/hooks/*.log`, seed scratch scripts the user did not ask to ship.
...
Or, if the Supabase MCP server is configured, `mcp__supabase__list_migrations` with `project_id: szkextipgpupqoppccoy`, then diff against files in `supabase/migrations/`.
...
Fallback: `mcp__supabase__apply_migration` with `name` (snake_case without timestamp) + full SQL `query` from the migration file — one migration at a time, in timestamp order.
...
Prefer **CLI** (correct multi-file bundles). Do **not** use `mcp__supabase__deploy_edge_function` unless CLI is unavailable — MCP requires manually shipping every file and easily misses `_shared`.
