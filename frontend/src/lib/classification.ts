...
function computePressureProfile(
  ...
  const loadSeverity: Record<string, number> = {
    manageable: 1, mild: 1, stable: 1, secure: 1,
    ...
    overwhelming: 4, severe: 4, crisis: 4, critical: 4,
  };

  const loadValues = Object.values(loadSignals).map((v) => loadSeverity[v] ?? 2);
  const avgLoad = loadValues.length > 0 ? loadValues.reduce((a, b) => a + b, 0) / loadValues.length : 2;

  const stateMap: Record<string, number> = {
    grounded: 1, wired: 3, depleted: 4, shut_down: 4,
    strong: 1, moderate: 2, low: 3,
  };
  const stateValues = Object.values(stateSignals).map((v) => stateMap[v] ?? 2);
  const avgState = stateValues.length > 0 ? stateValues.reduce((a, b) => a + b, 0) / stateValues.length : 2;
