...
import { generateAiReplyStub, type ChatAiProfileData } from "@/lib/chat/chatAiReplyStub";
...
export default function ChatPanelMount({
  ...
  const maybeSendSessionOpener = useCallback(
    async (loadedMessages: ChatMessage[]) => {
      ...
      try {
        const openingText = await requestSessionOpening(profileData, context);
  ...
  const finalizeSession = useCallback(
    async (thread: ChatMessage[]) => {
      const payload = await finalizeSessionFromThread(thread, profileData, context);
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        try {
          const assistantText = await generateAiReplyStub(threadForAi, context, profileData);
  ...
  const handleEndSession = useCallback(async () => {
    ...
    setAwaitingAssistantReply(true);
    try {
      const closeText = await requestSessionClose(messages, profileData, context);
