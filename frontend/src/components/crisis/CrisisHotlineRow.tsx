...
export function CrisisHotlineRow({
  ...
  return (
    ...
    >
      <span className={cn(bubbleStyle("Icon_primary_"), "mt-0.5 shrink-0")} aria-hidden>
        {icon}
      </span>
      <div className={cn(bubbleStyle("Group_transparent_"), "min-w-0 flex-col gap-0.5")}>
        <span
          className={cn(bubbleStyle("Text_label_"), "block text-sm font-semibold text-foreground")}
        >
          {name}
        </span>
        <span className={cn(bubbleStyle("Text_small_"), "block text-xs text-primary")}>
          {detail}
        </span>
      </div>
    </a>
