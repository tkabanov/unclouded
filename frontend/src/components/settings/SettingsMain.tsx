...
export default function SettingsMain() {
  ...
  useEffect(() => {
    ...
    if (tabParam === SETTINGS_TAB.SUBSCRIPTION) {
      if (activeTab !== SETTINGS_TAB.SUBSCRIPTION) {
        setActiveTab(SETTINGS_TAB.SUBSCRIPTION);
      }
      return;
    }
    ...
    next.set("tab", SETTINGS_TAB.SUBSCRIPTION);
  ...
  // Sync store from URL when the query changes (browser back/forward, deep links).
  // Do not depend on `activeTab`: after a tab click, state can update before `tab` clears
  // from the URL and would otherwise snap the UI back to the old tab.
  useEffect(() => {
    if (isSettingsTabSlug(tabParam) && tabs.includes(tabParam)) {
  ...
  }, [tabParam, tabs, isAdmin, setActiveTab, searchParams]);
  ...
  useEffect(() => {
    if (tabParam && (!isSettingsTabSlug(tabParam) || !tabs.includes(tabParam))) {
      ...
      if (activeTab === SETTINGS_TAB.PROFILE) {
        setSearchParams({}, { replace: true });
      } else {
        setSearchParams({ tab: activeTab }, { replace: true });
      }
    }
  }, [activeTab, setSearchParams, tabParam, tabs, searchParams]);
