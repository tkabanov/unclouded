-- NCLDD-31 §6 / CL-7 — Complete at scheduled end; decouple form submit; Kota sync stamps; admin form status.

ALTER TABLE public."coachBooking"
  ADD COLUMN IF NOT EXISTS "postSessionKotaSyncedAt" TIMESTAMPTZ NULL,
  ADD COLUMN IF NOT EXISTS "postSessionKotaSyncDetail" TEXT NULL;

COMMENT ON COLUMN public."coachBooking"."postSessionKotaSyncedAt" IS
  'When post-session notes were appended into the member chat_session_memory (Kota feed).';
COMMENT ON COLUMN public."coachBooking"."postSessionKotaSyncDetail" IS
  'Detail / error for the Kota session-memory sync attempt.';

-- Mark confirmed 1:1 sessions Completed once scheduled end has passed (independent of coach form).
CREATE OR REPLACE FUNCTION public.complete_ended_coach_bookings()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  WITH due AS (
    ...
    WHERE b.status = 'confirmed'
      ...
      AND b."scheduledAt"
        + make_interval(mins => COALESCE(b."durationMinutes", 30))
        <= now()
  ),
  updated AS (
    UPDATE public."coachBooking" b
    SET
      status = 'completed',
      "completedAt" = COALESCE(b."completedAt", now())
    FROM due
    WHERE b.id = due.id
    RETURNING b.id
  )
...
REVOKE ALL ON FUNCTION public.complete_ended_coach_bookings() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_ended_coach_bookings() TO service_role;

COMMENT ON FUNCTION public.complete_ended_coach_bookings() IS
  'NCLDD-31 §6 / CL-7: set confirmed 1:1 bookings to completed once scheduled end time has passed.';

-- Form submit stores notes only; status Completed is owned by complete_ended_coach_bookings.
...
COMMENT ON FUNCTION public.submit_coach_post_session(UUID, TEXT) IS
  'NCLDD-31 §6 / CL-7: store coach notes by token without gating booking Completed status.';

-- Admin list: expose postSessionFormStatus (pending | submitted | n/a).
    ...
    'rows', COALESCE((
      WITH one_on_one AS (
        SELECT
          ...
          CASE
            WHEN b."postSessionSubmittedAt" IS NOT NULL THEN 'submitted'
            ELSE 'pending'
          END AS "postSessionFormStatus",
          b."completedAt",
          b."createdAt",
          b."cancelledAt"
        FROM public."coachBooking" b
      ...
      group_rows AS (
        SELECT
          ...
          'n/a'::text AS "postSessionFormStatus",
