-- Repair REQ-15: ensure The Unsent Letter path has 4 pathSession + pathQuestion rows.
-- Idempotent — safe when original seed partially applied (path row without sessions).

DELETE FROM public."pathQuestion"
WHERE "sessionId" IN (
  SELECT id FROM public."pathSession"
  WHERE "pathId" = 'c8e1f0a2-4b3d-5e6f-9a0b-1c2d3e4f5a6b'
);

DELETE FROM public."pathSession"
WHERE "pathId" = 'c8e1f0a2-4b3d-5e6f-9a0b-1c2d3e4f5a6b';
