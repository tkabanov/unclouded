...
### OVR-065 — Deep-dive module completion lands on a qualitative results screen

| | |
|---|---|
| **Date** | 2026-09-09 |
| **Overrides** | `docs/deep-dive-modules-spec.md` §6 line 148 — flow defined as "intro → N questions → completion screen → redirect to Profile + toast", no results step; §5 line 138 — "Review" CTA marked post-MVP |
| **Authoritative spec** | Owner decision, `docs/module-results-screen-implementation-plan.md` (MRS-00 product lock) |
| **Current behavior** | Completing (or refreshing) any of the 6 deep-dive modules renders a **qualitative results screen** (`ModuleResultsScreen`) instead of the generic "answers have been saved" confirmation — no numeric score, no digit surfaced from `identityRoleFusionScore`. Reflection copy is derived from Build Brief §9 "WHAT THIS UNLOCKS" + question option labels, resolved purely from persisted profile fields (`resolveModuleResults`), so a module's completion view and its later Review view render identically. A "Newly unlocked paths" block (`resolveNewlyUnlockedPaths`) shows **all** genuinely-unlocked paths regardless of tier, labeling out-of-tier ones "Available on Pro" (per OVR-009 — no module-side tier gate). History & Context gets a softened variant: no "What this unlocks" bullets, no unlocked-paths block, no achievement framing — reflections only, with `CrisisBar` present via the existing `ModuleWizardShell`. Secondary CTA is "Back to profile" only (no chat link). A completed module's card gets a **Review** link to `/settings/know-yourself/:moduleSlug/results` — a read-only view of the same resolved results, no write. Refresh-mode submits land on the same results screen as initial completion. |
| **Code** | `frontend/src/lib/modules/results/` (`moduleResultsContent.ts`, `resolveModuleResults.ts`, `moduleUnlockedPaths.ts`), `frontend/src/lib/modules/moduleProfileFieldReader.ts`, `frontend/src/components/modules/ModuleResultsScreen.tsx`, `frontend/src/pages/ModuleWizard.tsx`, `frontend/src/pages/ModuleResultsPage.tsx`, `frontend/src/lib/router/authenticatedRoutes.tsx`, `frontend/src/components/settings/knowYourself/ModuleListCard.tsx` |

### OVR-066 — Mobile app wrapper: Capacitor shell, native purchase CTAs hidden

| | |
|---|---|
| **Date** | 2026-09-09 |
| **Overrides** | `docs/Uncloud360_Phase2_Requirements_v3.docx.md` §10 Option 2 — "Bubble wrapper" around the (obsolete) Bubble app; US-704 — native app "directs users to web checkout" (a tappable purchase link inside the WebView) |
| **Authoritative spec** | `docs/mobile-app-wrapper-implementation-plan.md` `MOB-00` product/platform lock (owner confirmed in session) |
